...
@props([
    ...
    'modalId2' => 'existingSkillQualificationModal',
])
...
<div class="mt-4">
    ...
    <div id="sortable-list" class="list-group">
        ...
        @foreach($jobTitle->skills as $skill)
            <div wire:ignore.self
                class="list-group-item d-flex align-items-center mb-2 border border-secondary py-2 px-3 rounded"
                draggable="true" ondragstart="handleDragStart(event, this, '{{ $skill->keyword_id }}')"
                ondragover="event.preventDefault()" ondrop="drop(event, '{{ $skill->keyword_id }}')" ondragend="handleDragEnd(this)">

                <div class="col-8">{{ $skill->keyword }}</div>

                <div class="col-2">
                    @php
                        $color = JobQualificationPriorityLevel::from($skill->priority)->getColor();
                        $label = JobQualificationPriorityLevel::from($skill->priority)->label();
                    @endphp
                ...
                <div class="col-2 d-flex justify-content-end">
                    ...
                    <button wire:click="openExistingEditMode( {{ $skill->keyword_id }} )" data-bs-toggle="tooltip"
                        class="btn no-hover-border me-2" data-bs-title="Edit">
                        <i class="icon p-1 mx-2 text-info" data-lucide="pencil"></i>
                    </button>

                    <button wire:click="removeExistingQualification( {{ $skill->keyword_id }} )" data-bs-toggle="tooltip"
                        class="btn no-hover-border me-2" data-bs-title="Delete">
                        <i class="icon p-1 mx-2 text-danger" data-lucide="trash-2"></i>
                    </button>
...
<script>
    ...
    Livewire.on('openSkillQualificationModal', () => {
        openModal('{{ $modalId }}');
    });

    Livewire.on('closeSkillQualificationModal', () => {
        hideModal('{{ $modalId }}');
        hideModal('{{ $modalId2 }}');
    });
    ...
    Livewire.on('openExistingSkillQualificationModal', () => {
        openModal('{{ $modalId2 }}');
    });
</script>
@endscript