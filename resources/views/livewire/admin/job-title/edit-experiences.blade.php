...
@props([
    ...
    'modalId2' => 'existingExpQualificationModal',
])

<div class="mt-4">

    <x-headings.form-snippet-intro label="{{ __('Experience') }}" :nonce="$nonce" required="true">
    ...
    <div id="sortable-list" class="list-group">
        ...
        @foreach($jobTitle->experiences as $exp)
            <div wire:ignore.self
                class="list-group-item d-flex align-items-center mb-2 border border-secondary py-2 px-3 rounded"
                draggable="true" ondragstart="handleDragStart(event, this, '{{ $exp->keyword_id }}')"
                ondragover="event.preventDefault()" ondrop="drop(event, '{{ $exp->keyword_id }}')" ondragend="handleDragEnd(this)">

                <div class="col-8">{{ $exp->keyword }}</div>

                <div class="col-2">
                    @php
                        $color = JobQualificationPriorityLevel::from($exp->priority)->getColor();
                        $label = JobQualificationPriorityLevel::from($exp->priority)->label();
                    @endphp
                ...
                <div class="col-2 d-flex justify-content-end">
                    ...
                    <button wire:click="openExistingEditMode( {{ $exp->keyword_id }} )" data-bs-toggle="tooltip"
                        class="btn no-hover-border me-2" data-bs-title="Edit">
                        <i class="icon p-1 mx-2 text-info" data-lucide="pencil"></i>
                    </button>

                    <button wire:click="removeExistingQualification( {{ $exp->keyword_id }} )" data-bs-toggle="tooltip"
                        class="btn no-hover-border me-2" data-bs-title="Delete">
                        <i class="icon p-1 mx-2 text-danger" data-lucide="trash-2"></i>
                    </button>
    ...
    <div x-data="{ showInput: false }" x-cloak>
        ...
        <button x-show="!showInput" @click="showInput = true" type="button"
            ...
            {{ __('Add Experience Qualifications') }}
        </button>
            ...
            <div class="row align-items-center py-3">
                ...
                <div class="col-7">
                    <input id="qualification-input" name="qualification" class="form-control border ps-3 rounded"
                        placeholder="{{ __(' E.g., 3 years in Customer Service') }}" wire:model="state.qualification"
                        autocomplete="off">
...
<script>
    ...
    Livewire.on('openExistingExpQualificationModal', () => {
        openModal('{{ $modalId2 }}');
    });
</script>
@endscript