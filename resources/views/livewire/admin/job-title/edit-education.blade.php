...
@props([
    'modalId' => 'qualificationsModal',
    'modalId2' => 'existingEducationQualificationModal',
])

<div class="mt-4">

    <x-headings.form-snippet-intro label="{{ __('Education') }}" :nonce="$nonce" required="true">
    ...
    <x-modals.dialog :id="$modalId2">
        ...
        <x-slot:footer wire:loading.attr="disabled">
            <button wire:click="discard" class="btn btn-secondary">{{ __('Close') }}</button>
            <button wire:click="saveExisting" class="btn btn-primary">{{ __('Save changes') }}</button>
        </x-slot:footer>
    </x-modals.dialog>
    ...
    <div id="sortable-list" class="list-group">
        @foreach($items as $index => $item)
            ...
            <div wire:ignore.self
                ...
                <div class="col-2 d-flex justify-content-end">
                    ...
                    <!-- Delete -->
                    <button wire:click="removeQualification( {{ $index }} )" data-bs-toggle="tooltip"
        ...
        @foreach($jobTitle->educations as $educ)
            <div wire:ignore.self
                class="list-group-item d-flex align-items-center mb-2 border border-secondary py-2 px-3 rounded"
                draggable="true" ondragstart="handleDragStart(event, this, '{{ $educ->keyword_id }}')"
                ondragover="event.preventDefault()" ondrop="drop(event, '{{ $educ->keyword_id }}')" ondragend="handleDragEnd(this)">

                <div class="col-8">{{ $educ->keyword }}</div>

                <div class="col-2">
                    @php
                        $color = JobQualificationPriorityLevel::from($educ->priority)->getColor();
                        $label = JobQualificationPriorityLevel::from($educ->priority)->label();
                    @endphp
                ...
                <div class="col-2 d-flex justify-content-end">
                    ...
                    <button wire:click="openExistingEditMode( {{ $educ->keyword_id }} )" data-bs-toggle="tooltip"
                        class="btn no-hover-border me-2" data-bs-title="Edit">
                        <i class="icon p-1 mx-2 text-info" data-lucide="pencil"></i>
                    </button>

                    <button wire:click="removeExistingQualification( {{ $educ->keyword_id }} )" data-bs-toggle="tooltip"
                        class="btn no-hover-border me-2" data-bs-title="Delete">
                        <i class="icon p-1 mx-2 text-danger" data-lucide="trash-2"></i>
                    </button>
    ...
    <div x-data="{ showInput: false }" x-cloak>
        ...
        <button x-show="!showInput" @click="showInput = true" type="button"
            ...
            {{ __('Add Education Qualifications') }}
        </button>
            ...
            <div class="row align-items-center py-3">
                ...
                <div class="col-7">
                    <input id="qualification-input" name="qualification" class="form-control border ps-3 rounded"
                        placeholder="{{ __('Graduate of any 4-year course.') }}" wire:model="state.qualification"
                        autocomplete="off">
...
<script>
    ...
    Livewire.on('openEducationQualificationModal', () => {
        openModal('{{ $modalId }}');
    });

    Livewire.on('closeEducationQualificationModal', () => {
        hideModal('{{ $modalId }}');
        hideModal('{{ $modalId2 }}');
    });
    ...
    Livewire.on('openExistingEducationQualificationModal', () => {
        openModal('{{ $modalId2 }}');
    });
</script>
@endscript