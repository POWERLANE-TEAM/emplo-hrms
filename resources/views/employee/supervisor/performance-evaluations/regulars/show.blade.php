    ...
    <x-headings.header-with-status title="{{ $performance->employeeEvaluatee->full_name }}" color="info" badge="Regular">
        <div class="d-flex position-relative">
            ...
            @if ($isEndpointAccessible)
                <form action="{{ route($routePrefix . '.performances.plan.improvement.regular.generate') }}" method="post"
                    class="d-contents">
                    @csrf
                    <input type="hidden" name="performance-form" value="{{ $performance->regular_performance_id }}">
                    <input type="hidden" name="employee-status"
                        value="{{ $performance->employeeevaluatee->status->emp_status_id }}">

                    <button type="submit" class="btn btn-primary rounded-circle py-2 px-1 ms-auto translate-middle-y"
                        data-bs-toggle="tooltip" data-bs-placement="left"
                        data-bs-title="Click to generate a performance improvement plan for {{ $performance->employeeevaluatee->first_name }}.">
                        <span class="m-2"><i class="icon icon-xxl " data-lucide="sparkle"></i></span>
                    </button>
                </form>
            @endif
        </div>
