...
<x-layout.employee.nav.sidebar.nav-group :sidebar_expanded="$sidebar_expanded" class="" txt_collapsed="Main" txt_expanded="Menu">
    ...
    @canAny([UserPermission::VIEW_EMPLOYEE_DASHBOARD, UserPermission::VIEW_HR_MANAGER_DASHBOARD, UserPermission::VIEW_ADMIN_DASHBOARD])
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.dashboard') }}" :active="request()->is($guard.'.dashboard')" class="tw-order-[0]" nav_txt="Dashboard"
    ...
    @can(UserPermission::VIEW_ALL_APPLICANTS)
        <x-layout.employee.nav.sidebar.nav-item href="#" :active="request()->is($guard.'.applicants')" class="tw-order-[2]"
    ...
    @can(UserPermission::VIEW_ACCOUNT_MANAGER)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.accounts') }}" :active="request()->routeIs($guard.'.accounts')" class="" nav_txt="Accounts"
            :default_icon="['src' => 'skill-evaluator', 'alt' => '']" :active_icon="['src' => 'skill-evaluator', 'alt' => '']">
        </x-layout.employee.nav.sidebar.nav-item>
    @endcan

    @can(UserPermission::VIEW_EMPLOYEE_MANAGER)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.employees') }}" :active="request()->routeIs($guard.'.employees')" class="" nav_txt="Employees"
            :default_icon="['src' => 'employee', 'alt' => '']" :active_icon="['src' => 'employee', 'alt' => '']">
        </x-layout.employee.nav.sidebar.nav-item>
    @endcan

    @can(UserPermission::VIEW_CALENDAR_MANAGER)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.calendar') }}" :active="request()->routeIs($guard.'.calendar')" class="" nav_txt="Calendar"
            :default_icon="['src' => 'calendar', 'alt' => '']" :active_icon="['src' => 'calendar', 'alt' => '']">
        </x-layout.employee.nav.sidebar.nav-item>
    @endcan

    @can(UserPermission::VIEW_JOB_LISTING_MANAGER)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.job-listing') }}" :active="request()->routeIs($guard.'.job-listing')" class="" nav_txt="Job Listing"
            :default_icon="['src' => 'job-listing', 'alt' => '']" :active_icon="['src' => 'job-listing', 'alt' => '']">
        </x-layout.employee.nav.sidebar.nav-item>
    @endcan

    @can(UserPermission::VIEW_POLICY_MANAGER)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.policy') }}" :active="request()->routeIs($guard.'.policy')" class="" nav_txt="Policies"
            :default_icon="['src' => 'policies', 'alt' => '']" :active_icon="['src' => 'policy', 'alt' => '']">
        </x-layout.employee.nav.sidebar.nav-item>
    @endcan

    @can(UserPermission::VIEW_ANNOUNCEMENT_MANAGER)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.announcement') }}" :active="request()->routeIs($guard.'.announcement')" class="" nav_txt="Announcements"
...
<x-layout.employee.nav.sidebar.nav-group :sidebar_expanded="$sidebar_expanded" class="" txt_collapsed="Configuration"
    ...
    @can(UserPermission::VIEW_PERFORMANCE_CONFIG)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.performance') }}" :active="request()->routeIs($guard.'.performance')" class="" nav_txt="Performance"
            :default_icon="['src' => 'performance', 'alt' => '']" :active_icon="['src' => 'performance', 'alt' => '']">
        </x-layout.employee.nav.sidebar.nav-item>
    @endcan

    @can(UserPermission::VIEW_FORM_CONFIG)
        <x-layout.employee.nav.sidebar.nav-item href="{{ route($guard.'.form') }}" :active="request()->routeIs($guard.'.form')" class="" nav_txt="Forms"
