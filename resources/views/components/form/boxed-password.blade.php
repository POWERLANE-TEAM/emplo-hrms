...
<div class="input-group mb-3">
    <div class="d-contents position-relative">
        @if (!empty($input_icon_left))
            <div class="px-2 d-flex position-absolute icon text-primary z-1" wire:ignore nonce="{{ $nonce }}">
                {{ $input_icon_left }}
            </div>
        @endif
        <input type="password"
            {{ $attributes->merge([
                    'class' => 'form-control rm-bg-icon border rounded z-0' . when(!empty($input_icon_left), 'ps-5'),
