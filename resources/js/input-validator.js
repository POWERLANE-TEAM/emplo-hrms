    ...
    validate(input_obj, callback) {
        ...
        if (input_element.validity.typeMismatch) {
            if (typeof callback === 'function') {
                callback(input_element, this.#validations?.errorFeedback?.typeMismatch);
            }
        ...
        try {
            if (this.#validations.attributes.type == 'email') {
                let emailDomain = this.extractDomain(original_value);

                if (!this.#validations.attributes.valid_emails.includes(emailDomain)) {
                    callback(input_element, this.#validations?.errorFeedback?.valid_emails);
                    return false;
                }
